...
typeConverter <- data.frame(masterType = unique(points$masterType),
                    classes = c('irrigated','dryland','irrigated','dryland',                                        'irrigated','dryland','irrigated','dryland',                                        'irrigated','dryland','noncrop','noncrop'))
...
```{r variableImportance3, fig.width = 3}
...
for (i in 1:length(seed20)) {
  set.seed(seed20[i])
  rf.fit <- randomForest(x = annualNoSoy[,1:20], # all variable columns 
                    y = as.factor(annualNoSoy$classes),
                    ntree = 500,
                    importance = TRUE)
  # get variable importance
  annual3[[i]] <- importance(rf.fit)[,4:5]
}

# reduce list by taking means
mean3 <- as.data.frame(Reduce(`+`, annual3) / length(annual3))

# format
mean3$variable <- rownames(mean3)
mean3long <- gather(mean3, key = metric, value = value, 
                    MeanDecreaseAccuracy:MeanDecreaseGini)

# plot
ggplot(mean3long[mean3long$metric == 'MeanDecreaseGini',],
       aes(x=reorder(variable, value), y = value)) +
  geom_bar(stat='identity') +
  coord_flip() +
  theme_bw() + ylab('Mean Decrease in GINI Index') + xlab('') +
  ggtitle('Annual 3 classes')

ggplot(mean3long[mean3long$metric == 'MeanDecreaseAccuracy',],
       aes(x=reorder(variable, value), y = value)) +
  geom_bar(stat='identity') +
  coord_flip() +
...
Run a binary classifier, since random forest chooses trees based on accuracy, so keeping 3 classes might affect this. 


Make a 2 class training dataset to run the random forest at the 2 class level
...
# plot
ggplot(mean2long[mean3long$metric == 'MeanDecreaseGini',],
       aes(x=reorder(variable, value), y = value)) +
  geom_bar(stat='identity') +
  coord_flip() +
  theme_bw() + ylab('Mean Decrease in GINI Index') + xlab('') +
  ggtitle('Annual 2 classes')

ggplot(mean2long[mean3long$metric == 'MeanDecreaseAccuracy',],
       aes(x=reorder(variable, value), y = value)) +
  geom_bar(stat='identity') +
  coord_flip() +
  theme_bw() + ylab('Mean Decrease in Accuracy') + xlab('') +
