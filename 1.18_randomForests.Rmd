...
Removing dryland soy does improve accuracies in 2007, but has no effect on irrigated or overall accuracies in 2010.
...
```{r reducedClassifier, fig.height=6}
...
## Test Datasets
Checked after settling on the random forest, 500 tree, all 57 variables variable method, run without soy training points. This gives accuracy from the R classification; accuracies will also be carried out on the GEE outputs (including 1x and interannual cleaning).
...
# clean up data
p2015 <- p2015[ , -which(names(p2015) %in% c('.geo'))]
p2015$class <- as.character(p2015$class)

p2002 <- p2002[ , -which(names(p2002) %in% c('.geo'))]
p2002$class <- as.character(p2002$class)

# collapse classes to 3 class, 2 class versions
keyTo3 <- data.frame(class = c('irrigated','dryland','noncrop','fallow'),
                     class3 = c('irrigated','dryland','noncrop','noncrop'),
                     class2 = c('irrigated','notirrigated','notirrigated','notirrigated'))
# incidentally, this also removes certainty level 3 as class 'Uncertain'
p2015 <- merge(p2015, keyTo3)
p2002 <- merge(p2002, keyTo3)

# subset certainty levels
p2015.certain <- p2015[p2015$certainty == 1,]
sum(p2015$certainty == 3)
p2015.all <- p2015[p2015$certainty %in% c(1,2),]  # removes completely uncertain points

p2002.certain <- p2015[p2015$certainty == 1,]
p2002.all <- p2015[p2015$certainty %in% c(1,2),]  # removes completely uncertain points



# run predictions/classification
p2015.certain$randFor <- predict(fit500.ns, newdata = p2015.certain, type='response')
p2015.all$randFor <- predict(fit500.ns, newdata = p2015.all, type='response')

# confusion tables
r2015.certain_3 <- makeConfusion(p2015.certain$randFor, p2015.certain$class3)
kable(r2015.certain_3, digits=1)

r2015.all_3 <- makeConfusion(p2015.all$randFor, p2015.all$class3)
kable(r2015.all_3, digits=1)
```
