...
see some nice updates in S:\Users\xutianfa\Irrigation\poster_files\code code


```{r knitrOpts, echo=FALSE}
library(knitr)
opts_chunk$set(cache=TRUE, cache.path='cache/4.1_FreqFigs/',
               fig.path='figure/4.1_FreqFigs_randFor/')
```
...
Frequency histogram of number of years irrigated for pixels

```{r hist, fig.width = 6, fig.height = 5, dpi=96, eval=FALSE}
rasDir <- 'C:/Users/deinesji/Google Drive/GEE_classification/RRB_test3_randFor_interannualClean'
rasAll <- 'IrrigationFrequencyMap_0and1neg_boundMask.tif'

# load full raster
freqHist <- raster(paste0(rasDir,'/',rasAll))
freqHist[freqHist == 0] <- NA

hist(freqHist, maxpixels=20000000, xlab = 'Years Irrigated', main = '')
...
```{r freqMap_matlab2_bottom, fig.width = 10, fig.height = 5, dpi=96}
rasDir <- 'C:/Users/deinesji/Google Drive/GEE_classification/RRB_test3_randFor_interannualClean'
rasAll <- 'IrrigationFrequencyMap_0and1neg_boundMask.tif'
...
image.plot(z=0:18,add=F,legend.only=T,  col=c('black',matlab), 
           horizontal = T,
...
blue-yellow-red Number of years irrigated
...
# color ramp 
spplot(freq_inset, col.regions = c('black',matlab), maxpixels = 200000,
       par.settings = list(layout.widths=list(right.padding=4.5)),
       #sp.layout=list(scale, text1, text2),
       colorkey=F)
...
colRamp <- c('black',matlab.like2(19))
