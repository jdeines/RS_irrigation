...
```{r stratFun}
# make a table for CDL weights for sampling (reduce grassland)
weightTable <- cdlKey[,1:2]
weightTable$weight <- 1 # standard
# urban and water: none
weightTable[122:125, 'weight'] <- 0 # developed classes
weightTable[112, 'weight'] <- 0 # open water
weightTable[82, 'weight'] <- 0 # clouds, no data
weightTable[83, 'weight'] <- 0 # developed
weightTable[84, 'weight'] <- 0 # water
# grassland: probability = 1/2
weightTable[177, 'weight'] <- 0.5
#

getStratCdl <- function(cdl.mask, poly, sampleDensity, cdlKey, seed=1,
                        p.weights = weightTable){
...
#   # add weights
   r.points <- merge(r.points, p.weights[,c('VALUE','weight')],
  ...
  # develop weights
  #cdlPresent <- r.points %>% distinct(CDLcode)

  cdlPresent <- r.points %>% 
                arrange(CDLcode) %>%
                group_by(CDLcode) %>%
                summarise()
  # set grassland to 50% prob
  cdlPresent$weights <- 1
  cdlPresent[cdlPresent$CDLcode == 176, 'weights'] <- 0.5 
  ...
  p1.r <- r.points %>%
          arrange(CDLcode) %>%
          group_by(CDLcode) %>%
          sample_frac(size = samFrac, weight = cdlPresent$weights)
...
# get points!
ptDensity.nrd <- 20 # 1 point per every "x" km2 

# generate stratified random sample
nrdPoints <- getStratCdl(cdl.mask.midRep, midRep, ptDensity.nrd, cdlKey, seed=1)
...
writeOGR(nrdPoints.ll, outdir, 'MidRep_20', driver='ESRI Shapefile', 
         overwrite = T)
writeOGR(nrdPoints.ll, paste0(outdir, '/MidRep_20.kml'),
            layer = 'thislayer', driver='KML', overwrite=T)

# write KML to my dropbox for laptop access
writeOGR(nrdPoints.ll, 'C:/Users/deinesji/Dropbox/1PhdJill/hpa/irrigation/data/GIS/kml/MidRep_20.kml', layer = 'thislayer', driver='KML', overwrite=T)
```
