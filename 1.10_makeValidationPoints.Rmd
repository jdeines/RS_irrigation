...
Because grassland is such a huge proportion of the landscape but relatively easy to classify as irr/nonirr, the function "downweights" this crop class. It does this in a gangsta way by increasingly the # points desired, and then removing half of the grassland points.
...
getStratCdl <- function(cdl.mask, poly, sampleDensity, cdlKey, seed=1){
  ...
  samFrac <- (pointsDesired*1.45)/ncells
...
#   dim(p1.r2)
#   table(p1.r2$CLASS_NAME)
  # remove half of grassland
  p1.r2$keep <- 1
  grassSamples <- sum(p1.r2$CLASS_NAME == 'Grass/Pasture')
  p1.r2[p1.r2$CLASS_NAME == 'Grass/Pasture','keep'] <- rbinom(grassSamples, 1, 0.5)  
...
writeOGR(nrdPoints.ll, outdir, 'MidRep_20_2010', driver='ESRI Shapefile', 
         overwrite = T)
writeOGR(nrdPoints.ll, paste0(outdir, '/MidRep_20_2010.kml'),
            layer = 'thislayer', driver='KML', overwrite=T)

# write KML to my dropbox for laptop access
writeOGR(nrdPoints.ll, 'C:/Users/deinesji/Dropbox/1PhdJill/hpa/irrigation/data/GIS/kml/MidRep_20_2010.kml', layer = 'thislayer', driver='KML', overwrite=T)
```
