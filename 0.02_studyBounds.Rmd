...
# write out the manually created RRCA bound to Dropbox
bound.wgs <- spTransform(bound, CRS("+proj=longlat +datum=WGS84"))
writeOGR(bound, outDir, 'RRCA_manual_aea', driver='ESRI Shapefile')
writeOGR(bound.wgs, outDir, 'RRCA_manual_wgs84', driver='ESRI Shapefile')

# write out the RRB to Dropbox for completeness
rrb.wgs <- spTransform(rrb, CRS("+proj=longlat +datum=WGS84"))
writeOGR(rrb, outDir, 'RRB_aea', driver='ESRI Shapefile')
writeOGR(rrb.wgs, outDir, 'RRB_wgs84', driver='ESRI Shapefile')

# sub-regions by state ----------------------------------------------------
# manual RRCA (hpa + RRCA) -----------------
...
rrcastates.wgs <- spTransform(rrcastates, CRS("+proj=longlat +datum=WGS84"))
writeOGR(rrcastates, outDir, 'RRCA_manual_byStates_aea', driver='ESRI Shapefile')
writeOGR(rrcastates.wgs, outDir, 'RRCA_manual_byStates_wgs84', driver='ESRI Shapefile')
...
spplot(rrbstates3, c('STATE_NAME'), col.regions=bpy.colors(4), main='basin split')

#export
rrbstates3.wgs <- spTransform(rrbstates3, CRS("+proj=longlat +datum=WGS84"))
writeOGR(rrbstates3, outDir, 'RRB_byState_KS_split_aea', driver='ESRI Shapefile')
writeOGR(rrbstates3.wgs, outDir, 'RRB_byStates_KS_split_wgs84', driver='ESRI Shapefile')

# RRCA-RRB intersection, by state
