...
colorbreaks <- 200
biPal <- c(blueRamp(colorbreaks/2),redRamp(colorbreaks/2))
...
idKey <- data.frame(masterid = c('RRB_KS_lowerBasin','Platte-NE_RRCA',
                   ...
                   masterid2 = c('gRRB_KS_lowerBasin', 'iPlatte-NE_RRCA',
                                 'fRRB_KS_upperBasin',
                   'dRRB_CO_basin', 'eRRB_NE_basin', 'hKS_non_RRB',
...
# plotty plot
labels <- c(`gRRB_KS_lowerBasin` = 'KS RRB (lower)',
            `iPlatte-NE_RRCA` = 'Platte RRCA',
            `fRRB_KS_upperBasin` = 'KS RRB (upper)',
...
botrow <- ggplot(data = regions[regions$rowname != 'one',],
    ...
    xlab('') + #ggtitle('Full Region by State') +
    scale_color_manual(breaks=c('total','novel','retired'),
                         labels=c('total', 'cumulative gained',
                                  'cumulative lost'),
                         values=c('red','blue','black')) +
    guides(color=guide_legend(title=NULL)) +
    theme_bw() +
...
dummy <- ggplot(data = regions[regions$rowname != 'one',],
  ...
  scale_fill_manual(values = c(CObasin, NEbasin, KSbasinUP,
                               KSlower, KSrrca, platte)) +
