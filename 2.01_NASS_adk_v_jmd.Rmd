...
# pad 5 digits column of geoID
adk3$fips5 <- sprintf("%05d",adk$GEOID)
# make a year column
adk3$year <- as.numeric(substr(as.character(adk$year), start = 1, stop = 4))
# subset to a single year
adk4 <- adk3[adk3$year == year,]
# just keep irrigated data
adk4 <- adk4[adk4$production_practice == 'IRRIGATED',]
...
# summarize by commodity and irrigation status for 2007
rrb2007 <- cleanNassCensusCounty(nass.df, year = year)

# just keep irrigated data and convert back to acres
# convert back to acres
rrb2007$irr_acres <- rrb2007$IRRIGATED * 247.105
rrb2007$scientist <- 'jmd'
rrb2007$year <- year
# columns to keep
jmd2 <- rrb2007[,c('fips5','year','commodity_desc','irr_acres','scientist')]
```
...
```{r commodityCompare}
# subset adk5 counties to match jmd2
adk6 <- adk5[adk5$fips5 %in% jmd2$fips5,]
# rename Jill's 'other hay' to Anthony's 'hay.other'
jmd2[jmd2$commodity_desc == 'OTHER HAY', 'commodity_desc'] <- 'HAY.OTHER'

# combine datasets
comm <- rbind(jmd2, adk6)

# sum acres by commodity type across counties
comm2 <- aggregate(irr_acres ~ commodity_desc + scientist, data = comm, FUN = 'sum')

# plot
ggplot(comm2, aes(commodity_desc,irr_acres)) +
  geom_bar(aes(fill = scientist), position = 'dodge', stat='identity') +
