...
```{r gridRegress_fun, fig.width = 3.5, fig.height = 3, dpi=600, dev=c('png','pdf')}
# load spatial bits
...
getLmCoeffs <- function(dataset, polygrid, normalize=TRUE){
  # drop columns
  ...
  data1$perc_irr <- data1$irrigated_m2/data1$area_m2
  ...
  if(normalize == TRUE){
    ...
    lms <- data1 %>% 
      group_by(polyid) %>% 
      do(model = lm(irrigated_m2 ~ year, data = .))
  } else {}  # use raw data
    lms <- data1 %>% 
    group_by(polyid) %>% 
  ...
  poly <- merge(polygrid, coeffs[,c('polyid','estimate','p.value')])
  # r
} 
