...
# keep only hay records with 'alfalfa' (otherwise overcounting!)
dropHay <- !(grepl('HAY',nass.df$short_desc)&!grepl('ALFALFA', nass.df$short_desc))
nass.df <- nass.df[dropHay,]
...
# combine hay and haylage by replacing commodity desc with hay
summary[grepl("HAY",summary$commodity_desc),'commodity_desc'] <- 'HAY'
# re-aggregate
summary2 <- aggregate(Value ~ commodity_desc + prodn_practice_desc,
                     data = summary, FUN='sum')
summary2

# fill in a non-irrigated value
summary.wide <- spread(summary2, key = prodn_practice_desc, value = Value)
summary.wide[is.na(summary.wide$IRRIGATED),'IRRIGATED'] <- 0
summary.wide$NON_IRR <- summary.wide$TOTAL - summary.wide$IRRIGATED
summary.wide[summary.wide$IRRIGATED == 0 ,'IRRIGATED'] <- NA

# make  a table
kable(summary.wide[,1:3])

# make a plot
summary.wide2 <- summary.wide[!is.na(summary.wide$IRRIGATED),
