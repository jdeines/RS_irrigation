...
cornprice <- read.csv(paste0(derivedDir, '/price_nass_corn_1996-2016.csv'))
...
# items to plot
plotUs <- c('Area_km2','Depth_m','Precip_mm','Price')
...
# cor plots
varcor <- cor(rrca, use='pairwise.complete.obs')
...
## cart without 2016 (how it looked before) ------------------------
 # control tree
controls <- ctree_control(minsplit = 2, minbucket = 2, mincriterion = .5)
...
# run tree
ct <- ctree(Area_km2 ~ Precip_mm + priceLag1 + farmBill, data = rrca, 
            controls = controls)
plot(ct, main="Conditional Inference Tree")
